...
class OrderAbs(val has_driver: Boolean,
  ...
  override def toString: String = {
    s"$has_driver $start_district_id $dest_district_id $price $year $month $day $hour $minute $second $time_id"
  }
...
object OrderAbs {
  ...
  def parse(s: String): OrderAbs = {
    val Array(has_d_s, sdi_s, ddi_s, p_s, y_s, m_s, d_s, hh_s, mm_s, ss_s, ti_s) = s.split(" ")
  ...
  def parse_order(s: String, districts_map: Map[String, Int]): OrderAbs = {
    ...
    new OrderAbs("NULL" != did,
      ...
      p_s.toDouble, y, m, d, hh, mm, ss, tid)
  }
