   ...
   dates.foreach{
     case (date_str,weekday,type_id)=>
       ...
       districtsType.values.toArray.filter{
          ...
          case (did,tp)=>
             gaps_map( did) = gaps_map.getOrElse(did, Array[(String,Double)]()) ++
   ...
   dates.foreach{
      date =>
        ...
        val feats = districts.values.toArray.sorted.flatMap { did =>
           val f1 = getFeat(stat_map(weekday),date, did)//stat_map(weekday).getOrElse(did,(0,0,0,0,0))
           Range(1, 145).map {
            tid =>
