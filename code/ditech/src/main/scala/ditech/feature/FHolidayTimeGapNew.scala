     ...
     case (date_str,type_id,weekday)=>
       val orders = OrderAbs.load_local( ditech16.data_pt + s"/order_data/order_data_$date_str",districts )
        ...
        Range(1,ditech16.max_time_id + 1).foreach {
          tid =>
            feat_map(tid) = feat_map.getOrElse(tid, Array[(String,Double)]()) ++
   ...
   dates.foreach{
      date =>
        ...
        val feats = districts.values.toArray.sorted.flatMap { did =>
          Range(1, 145).map {
            tid =>
              val f1 = getFeat(stat_map._1, date, tid)//stat_map._1.getOrElse(tid,(0,0,0,0,0))
              val f2 = getFeat(stat_map._2, date, tid)//stat_map._2.getOrElse(tid,(0,0,0,0,0))
