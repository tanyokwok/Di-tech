   ...
   dates.foreach{
      date =>
        ...
        val feats = districts.values.toArray.sorted.flatMap { did =>
          Range(1, 145).map {
            tid =>
              val f1 = stat_map._1.getOrElse((did,tid),(0,0))
              val f2 = stat_map._2.getOrElse((did,tid),(0,0))
              s"$did,$tid\t${f1._1},${f1._2},${f2._1},${f2._2}"
          }
        }
