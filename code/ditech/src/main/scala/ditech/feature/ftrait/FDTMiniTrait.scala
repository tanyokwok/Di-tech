...
trait FDTMiniTrait {
  ...
  def collect_order( ord:OrderAbs, fs:collection.mutable.Map[(Int,Int,Int), Double])
          ...
          case (did,tp)=>
            Range(1,ditech16.max_time_id + 1  ).foreach{
              tid =>
                Range(0,10).foreach{
                  mid =>
                    gaps_map( (did,tid) ) = gaps_map.getOrElse((did,tid), Array[Double]()) ++ Array(fs.getOrElse((did,tid,mid),0.0))
                }
            }
        }
