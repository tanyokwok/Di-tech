...
object FHolidayDistrictGapNew {
  ...
  def getFeatMap(dates:IndexedSeq[(String,Int,Int)]) = {
    val feat_map = collection.mutable.Map[Int ,Array[(String,Double)]]()
       ...
       val orders = OrderAbs.load_local( ditech16.data_pt + s"/order_data/order_data_$date_str",districts )
        ...
        orders.foreach { ord =>
         ...
         districtsType.values.toArray.filter{
          ...
          case (did,tp)=>
            feat_map( did ) = feat_map.getOrElse( did, Array[(String,Double)]()) ++
...
//  //      (fs_vec.mean, fs_vec.median, fs_vec.stdev )
  ...
  def getFeat(stat_map:collection.mutable.Map[Int ,Array[(String,Double)]],
              date:String, did:Int) ={
   ...
   dates.foreach{
      date =>
        ...
        val feats = districts.values.toArray.sorted.flatMap { did =>
           val f1 = getFeat(stat_map._1,date, did)//stat_map._1.getOrElse(did,(0,0,0,0,0))
           val f2 = getFeat(stat_map._2,date, did)//stat_map._2.getOrElse(did,(0,0,0,0,0))
           Range(1, 145).map {
            tid =>
