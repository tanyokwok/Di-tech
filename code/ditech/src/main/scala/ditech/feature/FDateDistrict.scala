...
object FDateDistrict {
  ...
  def getStatisticsByDate() ={
    ...
    val global_stat = collection.mutable.Map[Int, Array[Double]]()
    ...
    dates_arr.foreach{
     case (date_str, type_id)=>
        ...
        districtsType.values.toArray.filter{
          ...
          case (did,tp)=>
            global_stat( did ) = global_stat.getOrElse(did, Array[Double]()) ++ Array(fs.getOrElse(did,0.0))
        }

    }

    global_stat.mapValues{
      fs =>
//        println( "array size" + fs.size )
        val fs_vec = Vec( fs )
