...
class Order(val order_id: String,
            ...
            val time_id: Int,
            val fine_id: Int,
            val min_id:Int) extends Serializable {
  ...
  override def toString: String = {
    s"order_id($order_id)," +
      ...
      s"time_id($time_id),fine_id($fine_id),min_id($min_id)"
  }
}
...
object Order {
  ...
  def parse(s: String, districts_map: Map[String, Int]): Order = {
    ...
    val fine_id = (hh*60 + mm)/5 + 1
    val min_id =(hh*60 + mm) + 1

    new Order(oid, did, pid, sd, dd, districts_map.getOrElse(sd, -1), districts_map.getOrElse(dd, -1), p_s.toDouble, t, y, m, d, hh, mm, ss, tid, fine_id, min_id)
  }
