...
object ClusterMapPOIMerger {
  ...
  val map = IO.load(ditech16.cluster_pt + "/cluster_map_part2").map {
    line =>
  ...
  IO.load( ditech16.cluster_pt + "/cluster_map_part1").foreach{
    line =>
      val Array(district_hash, did) = line.split( "\t")
  ...
  def mergePOI(): Unit ={
   val poi_strs = IO.load( ditech16.poi_pt + "/poi_data_part2") ++
     IO.load( ditech16.poi_pt + "/poi_data_part1")
