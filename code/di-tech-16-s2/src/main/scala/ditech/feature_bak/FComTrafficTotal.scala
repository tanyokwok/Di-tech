...
import com.houjp.ditech16.feature.FTrafficTotal
import ditech.common.util.Directory

/**
  * Feature combine pregaps
  */
object FComTrafficTotal {

  def main(args:Array[String]): Unit ={
    val fs_names = Array(FTrafficTotal.getClass.getSimpleName.replace("$",""))
  ...
  def loadFeatures(date:String,fs_names:Array[String]): Array[(String, Array[Double])] = {
    val fs_mix = fs_names.map {
      fs_name =>
        val fs_fp = {
          ditech16.data_pt + s"/fs/$fs_name/${fs_name}_$date"
        }
