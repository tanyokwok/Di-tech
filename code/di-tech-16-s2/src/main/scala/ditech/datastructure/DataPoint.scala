...
object DataPoint {
  ...
  def run(fs_names:Array[String], train_pt:String, test_pt:String): Unit ={
    run(ditech16.offline_pt + "/train_time_slices",
      train_pt + "/train_key",
      train_pt + "/train_libsvm",
      fs_names)
    run(ditech16.offline_pt + "/test_time_slices",
      train_pt + "/test_key",
      train_pt + "/test_libsvm",
      fs_names)
    run(ditech16.offline_pt + "/val_time_slices1",
      train_pt + "/val_key1",
      train_pt + "/val_libsvm1",
      fs_names)
      run(ditech16.offline_pt + "/val_time_slices2",
      train_pt + "/val_key2",
      train_pt + "/val_libsvm2",
      fs_names)

    run(ditech16.online_pt + "/train_time_slices",
      test_pt + "/train_key",
      test_pt + "/train_libsvm",
      fs_names)

    run(ditech16.online_pt + "/test_time_slices",
      test_pt + "/test_key",
      test_pt + "/test_libsvm",
      fs_names)
    run(ditech16.online_pt + "/val_time_slices",
      test_pt + "/val_key",
      test_pt + "/val_libsvm",
      fs_names)
  }
 ...
 def loadFeatures(date:String,fs_names:Array[String], time_id:Map[Int,Int]): Array[((Int,Int), Array[Double])] = {
  ...
  def run(time_slice_fp: String,
    ...
    val time_slices = TimeSlice.load_old(time_slice_fp)
