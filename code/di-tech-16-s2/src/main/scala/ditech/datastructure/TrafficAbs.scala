...
class TrafficAbs(val did: Int,
  ...
  override def toString: String = {
      ...
      s"level3($level3)," +
      s"level4($level4)" +
      s"year($year),month($month),day($day),hour($hour),minute($minute),second($second),tid($tid),ntid($ntid)"
  }
}
...
object TrafficAbs {
  ...
  def parse(s: String, districts_map: Map[String, Int]): TrafficAbs = {
    ...
    val traffic = new TrafficAbs(did, level1,level2,level3,level4, y, m, d, hh, mm, ss, tid, ntid)
