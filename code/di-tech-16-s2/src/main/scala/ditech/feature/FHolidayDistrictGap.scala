       ...
       val orders = OrderAbs.load_local( ditech16.data_pt + s"/order_abs_data/order_data_$date_str" )
        ...
        orders.foreach { ord =>
         ...
         districtsType.values.toArray.filter{
          ...
          case (did,tp)=>
            feat_map( did ) = feat_map.getOrElse( did, Array[Double]()) ++ Array(fs.getOrElse( did,0.0))
        }
   ...
   dates.foreach{
      date =>
        ...
        val feats = districts.values.toArray.sorted.flatMap { did =>
           val f1 = stat_map._1.getOrElse(did,(0,0,0,0,0))
           val f2 = stat_map._2.getOrElse(did,(0,0,0,0,0))
           Range(1, 1440 + 1).map {
            ntid =>
