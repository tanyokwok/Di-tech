 ...
 val districts_fp = ditech16.data_pt + "/cluster_map/cluster_map"
  ...
  def run(data_pt: String,f_name:String): Unit = {
    ...
    dates.foreach { now_date =>
      ...
      val arrive_map: Map[Int, Map[(Int, Int), Double]] = Range(1,16).map{
        pre =>
    ...
    orders.foreach{
      e=>
       if (-1 != e.start_district_id && !e.has_driver){
        fs((e.start_district_id, e.new_time_id)) =
          fs.getOrElse((e.start_district_id, e.new_time_id), 0.0) + 1.0
      }
    }
    fs.toMap
  }
