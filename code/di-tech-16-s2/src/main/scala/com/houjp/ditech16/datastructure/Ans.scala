...
object Ans {
  ...
  def load(fp: String): Array[Ans] = {
    IO.load(fp).map { s =>
      ...
      new Ans(did_s.toInt, TimeSlice.parse_old(ts_s), gap_s.toDouble)
    }
  }

  def load_new(fp: String): Array[Ans] = {
    IO.load(fp).map {s =>
      val Array(did_s, ts_s, gap_s) = s.split(",")
      new Ans(did_s.toInt, TimeSlice.parse_new(ts_s), gap_s.toDouble)
    }
  }
}