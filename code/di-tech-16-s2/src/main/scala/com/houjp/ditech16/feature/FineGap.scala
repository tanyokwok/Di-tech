...
object FineGap extends Log {

  def main(args: Array[String]) {
    // 寻找往前 pre 个时间片的gap
    run(ditech16.data_pt,FineGap.getClass.getSimpleName.replace("$","") )
    ...
    dates.foreach { now_date =>
      ...
      val pregap_s = districts.values.toArray.sorted.flatMap { did =>
        Range(1, 1440 + 1).map { ntid =>
          val v1 = pregap1.getOrElse((did, ntid), 0.0)
          val v2 = pregap2.getOrElse((did, ntid), 0.0)
          val v3 = pregap3.getOrElse((did, ntid), 0.0)
          val v4 = pregap4.getOrElse((did, ntid), 0.0)
          val v5 = pregap5.getOrElse((did, ntid), 0.0)
