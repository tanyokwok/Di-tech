...
object FHolidayTimeGap extends Log{
     ...
     case (now_date,type_id,weekday)=>
       ...
       now_orders.foreach { e =>
        ...
        Range(1,ditech16.max_new_time_id + 1).foreach {
          ntid =>
            feat_map(ntid) = feat_map.getOrElse(ntid, Array[Double]()) ++ Array(fs.getOrElse(ntid, 0.0))
        }
    }
    feat_map.mapValues{
      fs =>
   ...
   dates.foreach{
      date =>
        ...
        val feats = districts.values.toArray.sorted.flatMap { did =>
          Range(1, 1440 + 1).map {
            ntid =>
              val f1 = stat_map._1.getOrElse(ntid,(0,0,0,0,0))
              val f2 = stat_map._2.getOrElse(ntid,(0,0,0,0,0))
