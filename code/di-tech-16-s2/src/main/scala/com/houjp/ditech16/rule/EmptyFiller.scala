...
object EmptyFiller {
  def main(args: Array[String]) {
    run(ditech16.offline_pt)
    run(ditech16.online_pt)
  }
  ...
  def run(data_pt: String): Unit = {
    ...
    val time_slices: Array[TimeSlice] = TimeSlice.load_old(time_slices_fp)
...
//    ans_rule = exp(ans_map, districts, time_slices)
  ...
  def run_online(): Unit = {
    val ans_fp = ditech16.online_pt + "/ans/ans.csv"
    ...
    val time_slices = TimeSlice.load_old(time_slices_fp)

    val districts_fp = ditech16.cluster_pt + "/cluster_map"
    val districts = District.load_local(districts_fp)

    val pre_gap_fill_fp = ditech16.online_pt + "/ans/20160526_fill_seg_model.csv"
