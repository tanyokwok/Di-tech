...
object FineTimeGap extends Log{
  ...
  def run(data_pt: String,feat_name:String): Unit = {
    ...
    dates.foreach { now_date =>
      ...
      val pregap1 = cal_pre_gap(pre_orders_abs, now_orders_abs, 0)
      val pregap2 = cal_pre_gap(pre_orders_abs, now_orders_abs, 5)
      val pregap3 = cal_pre_gap(pre_orders_abs, now_orders_abs, 10)
      val pregap4 = cal_pre_gap(pre_orders_abs, now_orders_abs, 15)
      val pregap5 = cal_pre_gap(pre_orders_abs, now_orders_abs, 20)

      val pregap_s = districts.values.toArray.sorted.flatMap { did =>
        Range(1, 1440 + 1).map { ntid =>
          val v1 = pregap1.getOrElse(ntid, 0.0)
          val v2 = pregap2.getOrElse(ntid, 0.0)
          val v3 = pregap3.getOrElse(ntid, 0.0)
          val v4 = pregap4.getOrElse(ntid, 0.0)
          val v5 = pregap5.getOrElse(ntid, 0.0)
  ...
  def cal_pre_gap(pre_orders: Array[OrderAbs], now_orders: Array[OrderAbs], off: Int): Map[Int, Double] = {
    val ntid_len = 1440
    ...
    pre_orders.foreach { e =>
      ...
      if (-1 != e.start_district_id &&
        ...
        Range(ntid_len + 1, e_ntid + 1).foreach { ntid =>
          fs(ntid - ntid_len) = fs.getOrElse( ntid - ntid_len, 0.0) + 1.0
        }
      }
    }
    ...
    now_orders.foreach { e =>
      ...
      if (-1 != e.start_district_id &&
        ...
        Range(s_ntid, math.min(e_ntid, ntid_len) + 1).foreach { ntid =>
          fs( ntid - 0) = fs.getOrElse( ntid - 0, 0.0) + 1.0
        }
      }
    }
